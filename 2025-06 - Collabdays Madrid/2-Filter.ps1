...
####
#Find User with FirstName = Timothy
...
$AllTimothyUsers=$AllUsers | Where-Object {$_.givenName -eq "Timothy"}
$AllTimothyUsers.count
...
$JamesUsers=Get-AzureResourcePaging -URL "https://graph.microsoft.com/v1.0/users?`$filter=givenName eq 'Timothy'" -AuthHeader $token_Header
$JamesUsers.count
...
$JamesUsers=Get-AzureResourcePaging -URL "https://graph.microsoft.com/v1.0/users?`$filter=startsWith(givenName, 'Timo')" -AuthHeader $token_Header
$JamesUsers.count
...
$JamesUsers=Get-AzureResourcePaging -URL "https://graph.microsoft.com/v1.0/users?`$filter=givenName ne 'Timothy'&`$count=true" -AuthHeader $token_Header
$JamesUsers.count
...
$GroupName="Collabdays Madrid 2025"
