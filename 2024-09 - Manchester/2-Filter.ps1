...
$AllWilliamUsers=$AllUsers | Where-Object {$_.givenName -eq "William"}
$AllWilliamUsers.count
...
$JamesUsers=Get-AzureResourcePaging -URL "https://graph.microsoft.com/v1.0/users?`$filter=givenName eq 'William'" -AuthHeader $token_Header
$JamesUsers.count
...
$JamesUsers=Get-AzureResourcePaging -URL "https://graph.microsoft.com/v1.0/users?`$filter=startsWith(givenName, 'Will')" -AuthHeader $token_Header
$JamesUsers.count
...
$JamesUsers=Get-AzureResourcePaging -URL "https://graph.microsoft.com/v1.0/users?`$filter=givenName ne 'William'&`$count=true" -AuthHeader $token_Header
$JamesUsers.count
...
$Out.value | Where-Object {$_.jobTitle -eq "M365 Manchester"}
($Out.value | Where-Object {$_.jobTitle -eq "M365 Manchester"}).count
...
$GetMemberURL = "https://graph.microsoft.com/v1.0/groups/$($Group.value.id)/members/microsoft.graph.user?`$count=true&`$filter=jobTitle eq 'M365 Manchester'"
$Out=Invoke-RestMethod -Method Get -Uri $GetMemberURL -Headers $token_Header
